...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const resultMessage = eligibilityResult.querySelector(".result-message");
  ...
  eligibilityForm.addEventListener("submit", function (e) {
    ...
    // Get form values
    const formData = {
      indianDirector: document.getElementById("indianDirector").value,
      dscDin: document.getElementById("dscDin").value,
      legalAge: document.getElementById("legalAge").value,
      documents: document.getElementById("documents").value,
      uniqueName: document.getElementById("uniqueName").value,
      legalBusiness: document.getElementById("legalBusiness").value,
    };

    // Check eligibility and display result
    const eligibilityCheck = checkEligibility(formData);
    displayResult(eligibilityCheck);
  });

  function checkEligibility(data) {
    let status = "success";
    let title = "You are Eligible!";
    let message =
      "Based on your inputs, you meet the basic requirements for company registration.";
    let requirements = [];

    // Check each requirement
    if (data.indianDirector === "no") {
      status = "error";
      requirements.push("At least one director must be an Indian resident");
    }

    if (data.dscDin === "no") {
      status = data.dscDin === "inProcess" ? "warning" : "error";
      requirements.push(
        "All directors must have DSC and DIN (If you do not have a DSC or DIN, we will provide one for you)."
      );
    }
    ...
    if (data.legalAge === "no") {
      status = "error";
      requirements.push(
        "All directors and shareholders must be above 18 years"
      );
    }

    if (data.documents === "no" || data.documents === "partial") {
      status = data.documents === "partial" ? "warning" : "error";
      requirements.push(
        "Complete address and identity proof for all directors is required"
      );
    }

    if (data.uniqueName === "no" || data.uniqueName === "notYet") {
      status = "warning";
      requirements.push(
        "Company name availability check is pending (If you have not checked the company name availability, we will check it for you)."
      );
    }

    if (
      data.legalBusiness === "no" ||
      data.legalBusiness === "needConsultation"
    ) {
      status = data.legalBusiness === "needConsultation" ? "warning" : "error";
      requirements.push(
        "Business activity must comply with Indian law (If you need a consultant, you will have to visit the FAQ section)."
      );
    }
    ...
    if (status === "error") {
      ...
      message =
        "There are some mandatory requirements that need to be fulfilled:";
    } else if (status === "warning") {
      title = "Almost There!";
      message =
        "You are close to being eligible. Please complete these remaining steps:";
    }
  ...
  function displayResult(result) {
    ...
    // Add new classes
    eligibilityResult.classList.add(result.status);
  ...
  const formInputs = eligibilityForm.querySelectorAll("select");
